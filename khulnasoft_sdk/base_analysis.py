...
class Analysis(metaclass=abc.ABCMeta):
    ...
    It requires an API connection to Khulnasoft.
    """

    def __init__(self, api: KhulnasoftApiClient = None):
        """
    ...
    @classmethod
    @abc.abstractmethod
    def from_analysis_id(cls, analysis_id: str, api: KhulnasoftApiClient = None) -> 'Analysis':
        raise NotImplementedError()
    ...
    def check_status(self) -> consts.AnalysisStatusCode:
        ...
        if not self.is_analysis_running():
            raise errors.KhulnasoftError('Analysis is not running')
        ...
        if response.status_code == HTTPStatus.OK:
            ...
            if result['status'] == consts.AnalysisStatusCode.FAILED.value:
                ...
                raise errors.KhulnasoftError('Analysis failed')
            ...
            raise errors.KhulnasoftError('Error in response status code:{}'.format(response.status_code))
    ...
    def _assert_analysis_finished(self):
        ...
        if self.status != consts.AnalysisStatusCode.FINISHED:
            raise errors.KhulnasoftError('Analysis not finished successfully')

    @classmethod
    def _create_analysis_from_response(cls, response: Response, api: KhulnasoftApiClient, analysis_id: str):
