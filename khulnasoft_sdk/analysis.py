...
class FileAnalysis(Analysis):
    """
    FileAnalysis is a class for analyzing files. It is a subclass of the BaseAnalysis class and requires an API connection to Khulnasoft.
    ...
    @classmethod
    def from_analysis_id(cls, analysis_id: str, api: KhulnasoftApiClient = None) -> Optional['FileAnalysis']:
        """
       ...
       :return: A FileAnalysis instance with the given analysis ID.
        """
        response = KhulnasoftApi(api or get_global_api()).get_file_analysis_response(analysis_id, True)
    ...
    def from_latest_hash_analysis(cls,
        ...
        """
        analysis_report = KhulnasoftApi(api or get_global_api()).get_latest_analysis(file_hash,
                                                                                  private_only,
                                                                                  composed_only,
...
def get_file_analysis_by_id(analysis_id: str, api: KhulnasoftApi = None) -> Optional[FileAnalysis]:
    return FileAnalysis.from_analysis_id(analysis_id, api)


@deprecated('This method is deprecated, use FileAnalysis.from_analysis_by_id instead to be explict')
def get_analysis_by_id(analysis_id: str, api: KhulnasoftApi = None) -> Optional[FileAnalysis]:
...
class UrlAnalysis(Analysis):
    """
    UrlAnalysis is a class for analyzing URLs. It is a subclass of the BaseAnalysis class and requires an API connection to Khulnasoft.
    ...
    def __init__(self, url: Optional[str] = None, api: KhulnasoftApiClient = None):
        """
    ...
    @classmethod
    def from_analysis_id(cls, analysis_id: str, api: KhulnasoftApiClient = None) -> Optional['UrlAnalysis']:
        """
       ...
       :return: A UrlAnalysis instance with the given analysis ID.
        """
        response = KhulnasoftApi(api or get_global_api()).get_url_analysis_response(analysis_id, True)
        return cls._create_analysis_from_response(response, api, analysis_id)
...
def get_url_analysis_by_id(analysis_id: str, api: KhulnasoftApi = None) -> Optional[UrlAnalysis]:
