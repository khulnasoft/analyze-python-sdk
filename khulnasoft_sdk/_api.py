...
class KhulnasoftApi:
    def __init__(self, api: KhulnasoftApiClient):
        self.api = api
    ...
    def get_url_result(self, url: str) -> dict:
        ...
        if 'error' in result:
            raise errors.KhulnasoftError(f'response error: {result["error"]}')
        ...
        if output_stream:
            ...
            if os.path.isdir(path):
                try:
                    ...
                    filename = 'KhulnasoftScanner.exe' if platform == 'windows' else 'KhulnasoftScanner'
